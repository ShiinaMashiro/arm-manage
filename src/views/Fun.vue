<template>
  <div>
    <div class="search-btn">
      <el-button size="small" type="primary" @click="$router.push('/home/fun/add')"  v-if="$store.getters.checkChangeAuth()">添加功能介绍</el-button>
    </div>
    ...
    <!-- 添加faq -->
    <el-dialog title="添加介绍大类" :append-to-body="true"
    ...
    <!-- 修改faq -->
    <el-dialog title="修改介绍大类" :append-to-body="true"
               :close-on-click-modal="false" :show-close="false" top="15vh"
...
<script>
  export default {
    ...
    methods: {
      ...
      getFAQList () {
        let that = this
        that.$post(that.$uri.help.funList, {}).then(res => {
          that.info = res
        })
      },
      /* 获取class列表 */
      ...
      async deleteFAQ (id) {
        let that = this
        await that.$post(that.$uri.help.funDelete, {id})
      },
      /* 批量删除FAQ */
      deleteFAQBatch () {
      ...
      addFaq() {
        ...
        that.$refs.form.validate(valid => {
          if (valid) {
            that.$post(that.$uri.help.funClassAdd, that.addInfo).then(res => {
              if (res.success) {
      ...
      /* 修改faq弹窗 */
      changeFaqPop(row) {
        this.$store.commit(this.$mutation.CHANGE_FUN, row)
        this.$router.push('/home/fun/change')
      },
      /* 修改faq */
      changeFaq() {
        let that = this
        that.$refs.changeForm.validate(valid => {
          if (valid) {
            that.$post(that.$uri.help.funClassModify, that.changeInfo).then(res => {
              if (res.success) {
