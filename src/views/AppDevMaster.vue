  ...
  <div>
    ...
    <div v-if="info.success" class="device-case border-all">
      <el-table ref="multipleTable" :data="info.list" :header-cell-style="{backgroundColor: '#efefef'}" stripe size="mini"
        ...
        <el-table-column prop="deviceIp" label="上传云机"></el-table-column>
        <el-table-column prop="remark" label="备注"></el-table-column>
        <el-table-column prop="operationStatus" label="备份状态">
          <template slot-scope="scope">{{scope.row.operationStatus | statusFilter}}</template>
        </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.operationStatus === 2 || scope.row.operationStatus === -5" type="text" size="small" @click.stop="uploadBackup(scope.row)">上传</el-button>
    ...
    <!-- 录入IP弹窗 -->
    <Drawer title="录入IP" :visible.sync="addIpPopShow" @handClick="restore">
      <div style="font-size: 12px" v-if="addIpInfo">
        <el-form ref="form" label-width="130px" label-position="left" style="display: flex;flex-direction: column;height: 100%">
          <div style="font-size: 16px;margin-bottom: 5px">备份信息</div>
          <el-form-item label="备份ID">{{addIpInfo.id}}</el-form-item>
          <el-form-item label="备份应用">{{addIpInfo.appName}}</el-form-item>
          <el-form-item label="备份来源云机">{{addIpInfo.deviceIp}}</el-form-item>
          <el-form-item label="备份备注">{{addIpInfo.remark}}</el-form-item>
    ...
    <Drawer title="恢复记录" :visible.sync="changeIpPopShow" :opt="false">
      <div style="font-size: 12px" v-if="changeIpInfo">
        <el-form ref="form" label-width="130px" label-position="left" style="display: flex;flex-direction: column;height: 100%">
          <div style="font-size: 16px;margin-bottom: 5px">备份信息</div>
          <el-form-item label="备份ID">{{changeIpInfo.id}}</el-form-item>
          <el-form-item label="备份应用">{{changeIpInfo.appName}}</el-form-item>
          <el-form-item label="备份来源云机">{{changeIpInfo.deviceIp}}</el-form-item>
          <el-form-item label="备份备注">{{changeIpInfo.remark}}</el-form-item>
          <div style="width: 100%;border-bottom: 1px solid #ddd"></div>
